...
func main() {
	...
	for i, challengeString := range fixedChallenges.Challenges {
		...
		solution, err := pc.SolveChallenge(challengeString)
		if err != nil {
			panic(err)
		}
	...
	logTime(fmt.Sprintf("%d fixed challenges correctly solved", len(fixedChallenges.Challenges)))

	challenge, err := pc.CreateChallenge(puzzles, difficulty)
	if err != nil {
		panic(err)
	}
